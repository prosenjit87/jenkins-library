...
func TestMavenInstall(t *testing.T) {
	...
	t.Run("Install a file", func(t *testing.T) {
		...
		err := InstallFile("app.jar", "pom.xml", "", &utils)
	...
	t.Run("Install files in a project", func(t *testing.T) {
		...
		if assert.Equal(t, 5, len(utils.Calls)) {
			...
			assert.Equal(t, mock.ExecCall{Exec: "mvn", Params: []string{"-Dfile=" + filepath.Join(".", "target", "foo.jar"), "-Dpackaging=jar", "-DpomFile=pom.xml", "-Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn", "--batch-mode", "install:install-file"}}, utils.Calls[3])
			assert.Equal(t, mock.ExecCall{Exec: "mvn", Params: []string{"-Dfile=" + filepath.Join(".", "target", "foo.war"), "-DpomFile=pom.xml", "-Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn", "--batch-mode", "install:install-file"}}, utils.Calls[4])
		}
	})
