...
func InstallFile(file, pomFile, m2Path string, utils Utils) error {
	if len(pomFile) == 0 {
...
func doInstallMavenArtifacts(options EvaluateOptions, utils Utils) error {
	...
	for _, pomFile := range pomFiles {
		...
		if packaging == "pom" {
			err = InstallFile("", pathToPomFile, options.M2Path, utils)
			if err != nil {
				return err
			}
		} else {
...
func installJarWarArtifacts(pomFile, dir string, options EvaluateOptions, utils Utils) error {
	...
	if finalName == "" {
		...
		err = InstallFile("", pomFile, options.M2Path, utils)
		if err != nil {
			return err
		}
		return nil
	...
	if originalJarExists {
		err = InstallFile(originalJarFile(dir, finalName), pomFile, options.M2Path, utils)
		if err != nil {
			return err
		}
	} else if jarExists {
		err = InstallFile(jarFile(dir, finalName), pomFile, options.M2Path, utils)
		if err != nil {
			return err
		}
	}
	...
	if warExists {
		err = InstallFile(warFile(dir, finalName), pomFile, options.M2Path, utils)
		if err != nil {
			return err
		}
	}
	...
	if classesJarExists {
		err = InstallFile(classesJarFile(dir, finalName), pomFile, options.M2Path, utils)
		if err != nil {
			return err
		}
	}
